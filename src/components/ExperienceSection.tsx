...
const experiences: ExperienceProps[] = [
  ...
  },
  {
    ...
    description: "Created responsive websites for clients using HTML, CSS, and JavaScript. Worked closely with designers to implement pixel-perfect designs."
  },
  {
    title: "Student Developer",
    company: "University Tech Lab",
    duration: "September 2022 — December 2022",
    description: "Assisted in developing educational tools for students. Gained hands-on experience with Python and database management."
  }
];

const ExperienceSection = () => {
  return (
    <section id="experience" className="py-24 relative overflow-hidden" style={{ background: 'var(--gradient-warm)' }}>
                ...
                <div className="pl-12 md:pl-0 md:w-1/2">
                  <div className="bg-white/70 backdrop-blur-sm border border-white/60 rounded-2xl p-6 md:p-7 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs uppercase tracking-widest text-portfolio-purple font-medium">{exp.duration}</span>
                    </div>
                    <h3 className="font-serif-display italic text-2xl text-portfolio-navy mb-1">{exp.title}</h3>
                    <p className="text-portfolio-navy/70 font-medium mb-3">{exp.company}</p>
                    <p className="text-portfolio-navy/70 text-sm leading-relaxed">{exp.description}</p>
                  </div>
                </div>
              </div>
            ))}
