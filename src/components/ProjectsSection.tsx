...
const projects: ProjectProps[] = [
  {
    ...
    description: "A brief description of the project, what it does, and your role in developing it.",
  ...
  },
  {
    ...
    description: "Another project description highlighting the key features and your contributions.",
  ...
  },
  {
    ...
    description: "A description of a third project, mentioning the problem it solves and the approach you took.",
...
const ProjectCard = ({ title, description, technologies, demoUrl, repoUrl, index }: ProjectProps & { index: number }) => (
  <div className="group relative bg-white rounded-2xl overflow-hidden border border-portfolio-mauve/20 shadow-md hover:shadow-2xl transition-all duration-500 hover:-translate-y-2">
    ...
    <div className="relative h-48 overflow-hidden bg-gradient-to-br from-portfolio-pink/40 via-portfolio-mauve/40 to-portfolio-purple/40">
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="font-serif-display italic text-7xl text-white/40">{String(index + 1).padStart(2, '0')}</span>
      </div>
    ...
    <div className="p-6">
      <h3 className="font-serif-display italic text-2xl mb-2 text-portfolio-navy">{title}</h3>
      <p className="text-portfolio-navy/70 mb-4 text-sm leading-relaxed font-light">{description}</p>

      <div className="flex flex-wrap gap-2 mb-5">
        {technologies.map((tech, i) => (
          <span key={i} className="px-2.5 py-1 bg-portfolio-cream text-portfolio-navy/80 text-xs rounded-full border border-portfolio-mauve/30">
            {tech}
          </span>
        ))}
      </div>
      ...
      <div className="flex gap-3 pt-4 border-t border-portfolio-mauve/20">
        {demoUrl && (
          <Button variant="ghost" size="sm" asChild className="text-portfolio-navy hover:bg-portfolio-pink/20">
...
const ProjectsSection = () => {
  return (
    <section id="projects" className="py-24 relative overflow-hidden" style={{ background: 'var(--gradient-warm)' }}>
